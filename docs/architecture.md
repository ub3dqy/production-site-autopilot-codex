...
4. **Delivery** — plugin layout, fallback installers, deterministic archives, checksums, SBOM, provenance, and canonical local verification.
...
The runtime and release verification have no required network or GitHub Actions dependency. External operations are represented as policy actions and require authorization before an executor may perform them.
